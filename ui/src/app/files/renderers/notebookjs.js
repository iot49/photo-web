// Browser-compatible version of notebookjs
// Based on notebookjs v0.8.3 - http://github.com/jsvine/notebookjs
// Modified to work in browser without Node.js dependencies

const createNotebookJS = function () {
    var VERSION = "0.8.3";
    var root = this || globalThis;
    var isBrowser = true; // Force browser mode
    var doc = document; // Use browser document directly
    ...
    // Get supporting libraries - simplified for browser
    var getMarkdown = function () {
        return root.marked && root.marked.parse;
    };

    var getAnsi = function () {
        return root.ansi_up && root.ansi_up.ansi_to_html;
    };

    var getSanitizer = function () {
        return root.DOMPurify && root.DOMPurify.sanitize;
    };
    ...
    nb.Cell.prototype.renderers = {
        markdown: function () {
            ...
            // Create zero-md element for markdown rendering
            var zeroMd = doc.createElement("zero-md");
            
            // Create script element with markdown content
            var script = doc.createElement("script");
            script.type = "text/markdown";
            script.textContent = joined;
            
            zeroMd.appendChild(script);
            el.appendChild(zeroMd);
...
const nb = createNotebookJS();

// ES6 exports
export default nb;
export const parse = nb.parse;
export const VERSION = nb.VERSION;